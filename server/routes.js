...
async function search(req, res) {
    ...
    if (req.query.songName && !req.query.artistName) {
        ...
        query += "WITH tracks_alb (track_name, alb_id, track_id) as (SELECT name, album_id, id from tracks),     tracks_alb_art_id (track_name, t_id, art_id) as (SELECT tracks_alb.track_name, tracks_alb.track_id, artist_id from tracks_alb join albums on tracks_alb.alb_id = albums.id),     tracks_alb_art_na (track_name, t_id, gen_id) as (SELECT tracks_alb_art_id.track_name, tracks_alb_art_id.t_id, artists.name from tracks_alb_art_id join artists on tracks_alb_art_id.art_id = artists.id)  select track_name as name, gen_id as artists_name, t_id from tracks_alb_art_na where gen_id like '%" + req.query.artistName + "%';"
        ...
        query += "WITH tracks_alb (track_name, alb_id, track_id) as (SELECT name, album_id, id from tracks),     tracks_alb_art_id (track_name, t_id, art_id) as (SELECT tracks_alb.track_name, tracks_alb.track_id, artist_id from tracks_alb join albums on tracks_alb.alb_id = albums.id),     tracks_alb_art_na (track_name, t_id, gen_id) as (SELECT tracks_alb_art_id.track_name, tracks_alb_art_id.t_id, artists.name from tracks_alb_art_id join artists on tracks_alb_art_id.art_id = artists.id)  select track_name as name, gen_id as artists_name, t_id from tracks_alb_art_na where gen_id like '%" + req.query.artistName + "%' and track_name like '%" + req.query.songName + "%';"
